...
Call ALL four data-gathering tools before making any recommendation:
1. get_booking_metrics — reads occupancy, popular cars, recent bookings from Google Sheets
2. analyze_past_campaigns — reads past campaigns with result_score >= 8 to identify
   best-performing offers, audiences, and budget ranges. Learn from what worked before.
3. search_relevant_ads(query, city, country) — searches the web for relevant rental car
   ads, offers, and pricing. Pass the city and country from the market context.
4. search_market_trends(query, city, country) — searches the web for local car rental market
   trends, seasonal events, demand patterns. Pass the city and country from the market context.

Wait for all four responses before proceeding.
"""
