...
Extract facts — pull specific numbers, offers, observations from each tool result
  ↓
Detect conflicts — check if booking data, competitor ads, and trends agree or contradict
  ↓
Mark missing data — note what evidence is absent (no competitor pricing? no booking trend?)
  ↓
...
[SOURCE: competitor_search] — data from search_competitor_ads tool
[SOURCE: market_trends] — data from search_market_trends tool

SOURCE VISIBILITY RULE:
Every external factual claim (competitor offers, price ranges, market trends) must be
...
4. If competitor/trend data is unavailable, lower confidence accordingly.
5. If tool outputs disagree (e.g. booking data shows low demand but competitor ads suggest
